...
{            // this is called every 20 ms
    ...
    if (item == NULL) 
    {
            GetPixels(0, spectrumBuffer, &flag);
    return;               // no clients
    }
...
//        GetNAPixels(2, spectrumBuffer, &flag);
    ...
    else 
    {
        ...
        case PANADAPTER:
            ...
            GetNAPixels(0, spectrumBuffer, &flag);
            break;
...
//            Process_Panadapter(0, spectrumBuffer);
     ...
     //   meter = CalculateRXMeter(0, 0, rxMeterMode) + multimeterCalibrationOffset + getFilterSizeCalibrationOffset(); // Rx meter mode added by KD0OSS
        meter = (float)GetRXAMeter(0, RXA_S_AV) + multimeterCalibrationOffset + getFilterSizeCalibrationOffset();
     //   subrx_meter = CalculateRXMeter(0, 1, rxMeterMode) + multimeterCalibrationOffset + getFilterSizeCalibrationOffset(); // Rx meter mode added by KD0OSS
        subrx_meter = GetRXAMeter(1, RXA_S_AV) + multimeterCalibrationOffset + getFilterSizeCalibrationOffset();
    }
            ...
            for (j=0;j< data.output_frames_gen;j++) {
                ...
                if (iq_buffer_counter >= TX_BUFFER_SIZE) {
                   ...
                   /////     Audio_Callback(tx_buffer, &tx_buffer[TX_BUFFER_SIZE], tx_IQ_buffer, &tx_IQ_buffer[TX_BUFFER_SIZE], TX_BUFFER_SIZE, 1);
                        int error;
                        fexchange2(2, tx_buffer, &tx_buffer[TX_BUFFER_SIZE], tx_IQ_buffer, &tx_IQ_buffer[TX_BUFFER_SIZE], &error);
...
//                        samples_per_frame*24 : MIC_ALAW_BUFFER_SIZE*24 ;
            data.output_frames = MIC_ALAW_BUFFER_SIZE*24;
            data.src_ratio = mic_src_ratio;
            data.end_of_input = 0;
            ...
            if (rc) {
                ...
                //////            Audio_Callback(tx_buffer, &tx_buffer[TX_BUFFER_SIZE], &tx_IQ_buffer[TX_BUFFER_SIZE*2], &tx_IQ_buffer[TX_BUFFER_SIZE*3], TX_BUFFER_SIZE, 1);
                            fexchange2(2, tx_buffer, &tx_buffer[TX_BUFFER_SIZE], &tx_IQ_buffer[TX_BUFFER_SIZE*2], &tx_IQ_buffer[TX_BUFFER_SIZE*3], &error);
                        } else if (!hpsdr) {
                            // use DttSP to process Mic data into tx IQ
                  //////          Audio_Callback(tx_buffer, &tx_buffer[TX_BUFFER_SIZE], tx_IQ_buffer, &tx_IQ_buffer[TX_BUFFER_SIZE], TX_BUFFER_SIZE, 1);
                            fexchange2(2, tx_buffer, &tx_buffer[TX_BUFFER_SIZE], tx_IQ_buffer, &tx_IQ_buffer[TX_BUFFER_SIZE], &error);
       ...
       //     SetMode(1,0,mode);
        ...
        //        SetTXFilter(1,150,2850);
            ...
            case LSB:
                ...
                SetRXAMode(1, RXA_LSB);
                SetTXAMode(2, TXA_LSB);
                SetTXABandpassFreqs(2, -2850, -150);
        ...
        //        SetTXFilter(1,-2850, -150);
            ...
            case AM:
                ...
                SetRXAMode(1, RXA_AM);
                SetTXAMode(2, TXA_AM);
                SetTXABandpassFreqs(2, -2850, 2850);
                break;
            case SAM:
                SetRXAMode(0, RXA_SAM);
                SetRXAMode(1, RXA_SAM);
                SetTXAMode(2, TXA_SAM);
                SetTXABandpassFreqs(2, -2850, 2850);
            // SetTXFilter(1, -2850, 2850); 
            break;
            case FM:
                SetRXAMode(0, RXA_FM);
              //  SetRXAMode(1, RXA_FM);
                SetTXAMode(2, TXA_FM);
                SetTXABandpassFreqs(2, -4800, 4800);
            // SetTXFilter(1, -4800, 4800); 
            break;
            case DSB:
                SetRXAMode(0, RXA_DSB);
                SetRXAMode(1, RXA_DSB);
                SetTXAMode(2, TXA_DSB);
                SetTXABandpassFreqs(2, 150, 2850);
            break;
            case CWU:
                SetRXAMode(0, RXA_CWU);
                SetRXAMode(1, RXA_CWU);
                SetTXAMode(2, TXA_CWU);
                SetTXABandpassFreqs(2, 150, 2850);
            break;
            case CWL:
                SetRXAMode(0, RXA_CWL);
                SetRXAMode(1, RXA_CWL);
                SetTXAMode(2, TXA_CWL);
                SetTXABandpassFreqs(2, -2850, -150);
            break;
            case DIGU:
                SetRXAMode(0, RXA_DIGU);
                SetRXAMode(1, RXA_DIGU);
                SetTXAMode(2, TXA_DIGU);
                SetTXABandpassFreqs(2, 150, 2850);
            break;
            case DIGL:
                SetRXAMode(0, RXA_DIGL);
                SetRXAMode(1, RXA_DIGL);
                SetTXAMode(2, TXA_DIGL);
                SetTXABandpassFreqs(2, -2850, -150);
            break;
            case SPEC:
                SetRXAMode(0, RXA_SPEC);
                SetRXAMode(1, RXA_SPEC);
                SetTXAMode(2, TXA_SPEC);
                SetTXABandpassFreqs(2, 150, 2850);
            break;
            case DRM:
                SetRXAMode(0, RXA_DRM);
                SetRXAMode(1, RXA_DRM);
                SetTXAMode(2, TXA_DRM);
                SetTXABandpassFreqs(2, 150, 2850);
            break;
          ...
          //  SetRXFilter(0,1,(double)low,(double)high);
            SetRXABandpassFreqs(0, (double)low, (double)high);
            SetRXABandpassFreqs(1, (double)low, (double)high);
            sdr_log(SDR_LOG_INFO, "RX filter set: Low=%d High=%d\n", low, high);
#ifdef _OLD            
...
//            SetFixedAGC(0,0,agc);
//            SetFixedAGC(0,1,agc);
            SetRXAAGCFixed(0, (double)agc);
            SetRXAAGCFixed(1, (double)agc);
#ifdef _OLD            
          ...
          //  SetRXAGCThresh(0,1,(double)level);
            SetRXAAGCThresh(0, (double)level, 4096.0, (double)sampleRate);
            SetRXAAGCThresh(1, (double)level, 4096.0, (double)sampleRate);          
...
//            SetGrphRXEQ(0,1,(int *)&value);
            SetRXAGrphEQ(0, (int *)&value);
            SetRXAGrphEQ(1, (int *)&value);
...
//            SetGrphRXEQ10(0,1,(int *)&value);
            SetRXAGrphEQ10(0, (int *)&value);
            SetRXAGrphEQ10(1, (int *)&value);
...
//            SetRXOutputGain(0,0,(double)gain/100.0);
            SetRXAPanelGain1(0, (double)gain/100.0);
...
//            SetRXOutputGain(0,1,(double)gain/100.0);
            SetRXAPanelGain1(1, (double)gain/100.0);
...
//            SetRXOsc(0,1,offset - LO_offset);
            SetRXAShiftFreq(1, offset - LO_offset);
...
//            SetANFvals(0,1,taps,delay,gain,leakage);
            SetRXAANFVals(0, taps, delay, (double)gain, (double)leakage);
            SetRXAANFVals(1, taps, delay, (double)gain, (double)leakage);
...
//            SetANRvals(0,1,taps,delay,gain,leakage);
            SetRXAANRVals(0, taps, delay, (double)gain, (double)leakage);
            SetRXAANRVals(1, taps, delay, (double)gain, (double)leakage);
...
//            SetRXAGCAttack(0,1,value);
            SetRXAAGCAttack(0, value);
            SetRXAAGCAttack(1, value);
...
//            SetRXAGCDecay(0,1,value);
            SetRXAAGCDecay(0, value);
            SetRXAAGCDecay(1, value);
...
//            SetRXAGCHang(0,1,value);
            SetRXAAGCHang(0, value);
            SetRXAAGCHang(1, value);
...
//            SetRXAGCHangLevel(0,1,value);
            SetRXAAGCHangLevel(0, value);
            SetRXAAGCHangLevel(1, value);
...
//            SetRXAGCHangThreshold(0,1,value);
            SetRXAAGCHangThreshold(0, value);
            SetRXAAGCHangThreshold(1, value);
...
//            SetRXAGCThresh(0,1,value);
            SetRXAAGCThresh(0, value, 4096.0, sampleRate);
            SetRXAAGCThresh(1, value, 4096.0, sampleRate);
...
//            SetRXAGCTop(0,1,value);
            SetRXAAGCTop(0, value);
            SetRXAAGCTop(1, value);
...
//            SetRXAGCSlope(0,1,value);
            SetRXAAGCSlope(0, value);
            SetRXAAGCSlope(1, value);
#ifdef _OLD            
...
//            SetBIN(0,0,state);
//            SetBIN(0,1,state);
            SetRXAPanelBinaural(0, state);
            SetRXAPanelBinaural(1, state);
...
//            SetTXCompressor(1, value);
            SetTXACompressorGain(2, (double)value);
...
//            SetWindow(0,mode);
            SetRXABandpassWindow(0, mode);
            SetRXABandpassWindow(1, mode);
            SetTXABandpassWindow(2, mode);
            ...
            if (tokenize_cmd(&saveptr, tokens, 2) != 2)
                goto badcommand;
            sdr_log(SDR_LOG_INFO, "Spectrum fps set to = '%s'  Samples = '%s'\n", tokens[1], tokens[0]);
            sem_wait(&bufferevent_semaphore);
            if (slave) {
            ...
            initAnalyzer(0, item->samples, atoi(tokens[1]));
            sem_post(&bufferevent_semaphore);
...
#pragma omp for schedule(static)
        ...
        /*
          ...
          */  
            client_samples[i+BUFFER_HEADER_SIZE]=(unsigned char)-(rotated_samples[i]+extras);
        }
    }
}
...
void initAnalyzer(int disp, int pixels, int frame_rate)
{
...
printf("low: %d  high: %d  clip: %d  sclip_l: %d    sclip_h: %d  overlap: %d  maxw: %d\n", low, high, clip, span_clip_l, span_clip_h, overlap, max_w);
