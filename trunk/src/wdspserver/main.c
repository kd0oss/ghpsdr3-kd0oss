...
const char *version = "20141106;-master"; //YYYYMMDD; text desc
...
*/    
    WDSPwisdom(strcat(directory, "/"));
    printf("Wisdom set.\n");
    OpenChannel(0, 1024, 4096, 48000, 48000, 48000, 0, 0, 0.010, 0.025, 0.000, 0.010); // 0,0
    printf("Channel 0 opened.\n");fflush(stdout);
    OpenChannel(1, 1024, 4096, 48000, 48000, 48000, 0, 0, 0.010, 0.025, 0.000, 0.010); // 0,1
    printf("Channel 1 opened.\n");fflush(stdout);
    OpenChannel(2, 1024, 4096, 48000, 48000, 48000, 1, 0, 0.010, 0.025, 0.000, 0.010); // 2,0
    printf("Channel 2 opened.\n");fflush(stdout);
    ...
    int rc = 0;
    XCreateAnalyzer(0, &rc, 262144, 1, 3, strcat(directory, "/specWisdom"));
    if (rc < 0) printf("XCreateAnalyzer failed on channel 0.\n");
    XCreateAnalyzer(1, &rc, 262144, 1, 1, strcat(directory, "/specWisdom"));
    if (rc < 0) printf("XCreateAnalyzer failed on channel 1.\n");
    create_anbEXT(0, 1, 1024, 48000, 0.0001, 0.0001, 0.0001, 0.05, 20);
    create_anbEXT(1, 1, 1024, 48000, 0.0001, 0.0001, 0.0001, 0.05, 20);
    create_nobEXT(0, 1, 0, 1024, 48000, 0.0001, 0.0001, 0.0001, 0.05, 20);
    create_nobEXT(1, 1, 0, 1024, 48000, 0.0001, 0.0001, 0.0001, 0.05, 20);
    SetRXASpectrum(0, 0, 0, 0, 0);
    SetRXASpectrum(1, 0, 0, 0, 0);
    initAnalyzer(0, 128, 15);  // sample (128) will get changed to spectrum display width when client connects.
    initAnalyzer(1, 128, 15);  // sample (128) will get changed to spectrum display width when client connects.
    SetRXABandpassFreqs(0, 150, 2850);
    SetRXABandpassFreqs(1, 150, 2850);
    SetRXABandpassRun(0, 1);
    SetRXABandpassRun(1, 1);
    SetRXAShiftFreq(0, LO_offset);
    SetRXAShiftFreq(1, LO_offset);
    SetRXAAMSQThreshold(0, -150.0);    
    SetRXAAMSQThreshold(1, -150.0);    
    SetRXAAMSQRun(0, false);
    SetRXAAMSQRun(1, false);
    SetRXAAGCFixed(0, 20.0);
    SetRXAAGCFixed(1, 20.0);
    SetRXAAGCTop(0, 90.0);
    SetRXAAGCTop(1, 90.0);
  //  SetRXAPanelGain1(0, 0.5);
  //  SetRXAPanelPan (0, 0.5);
