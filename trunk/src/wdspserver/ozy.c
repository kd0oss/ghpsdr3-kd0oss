...
//    Spectrum(0, 0, 0, input_buffer, &input_buffer[BUFFER_SIZE]);
    ...
    while (1) 
    {
     ...
     //   Audio_Callback(input_buffer, &input_buffer[BUFFER_SIZE], output_buffer, &output_buffer[BUFFER_SIZE], buffer_size, 0);
        Spectrum(0, 0, 0, input_buffer, &input_buffer[BUFFER_SIZE]);
        int error;
        fexchange2(0, input_buffer, &input_buffer[BUFFER_SIZE], output_buffer, &output_buffer[BUFFER_SIZE], &error);
        if (error != 0) printf("error: %d\n", error);
 //       printf("Audio: %2.8f %2.8f %2.8f %2.8f\n", output_buffer[0], output_buffer[1], output_buffer[2], output_buffer[3]); 
...
#pragma omp parallel for schedule(static) private(j)
        ...
        if (hpsdr) 
        {
          ...
          //                          &output_buffer[BUFFER_SIZE*2], &output_buffer[BUFFER_SIZE*3], buffer_size, 1);
                fexchange2(2, &input_buffer[BUFFER_SIZE*2], &input_buffer[BUFFER_SIZE*3], &output_buffer[BUFFER_SIZE*2], &output_buffer[BUFFER_SIZE*3], &error);
            } 
            else 
            {
                    //
...
int ozySetOpenCollectorOutputs(char* state) 
{
    ...
    return result;
} // end ozySetOpenCollectorOutputs
...
{
    local_audio=state;
} // end ozy_set_local_audio


void ozy_set_port_audio(int state) 
{
    port_audio=state;
} // end ozy_set_port_audio
